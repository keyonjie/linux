...
static inline struct ivpu_device *seq_to_ivpu(struct seq_file *s)
{
	...
	return to_ivpu_device(node->minor->dev);
#endif
}
...
static int hws_log_show(struct seq_file *s, void *v)
{
	struct drm_printer p = drm_seq_file_printer(s);
	struct ivpu_device *vdev = seq_to_ivpu(s);

	ivpu_hws_log_print(vdev, &p);
	return 0;
}
...
static int fw_log_show(struct seq_file *s, void *v)
{
	...
	ivpu_fw_log_direct_print(vdev, true, &p);
	return 0;
}
...
static ssize_t
ivpu_force_dct_fn(struct file *file, const char __user *user_buf, size_t size, loff_t *pos)
{
	struct ivpu_device *vdev = file->private_data;
	u32 dct_val;
	int ret;

	ret = kstrtou32_from_user(user_buf, size, 0, &dct_val);
	if (ret < 0)
		return ret;
...
static const struct file_operations ivpu_force_dct_fops = {
	.owner = THIS_MODULE,
	.open = simple_open,
	.write = ivpu_force_dct_fn,
};
...
void ivpu_debugfs_init(struct drm_minor *minor)
{
	...
	debugfs_create_file("force_dct", 0200, debugfs_root, vdev,
			    &ivpu_force_dct_fops);

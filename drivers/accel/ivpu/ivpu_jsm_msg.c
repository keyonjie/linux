...
#define DCT_ACTIVE_US 5300
#define DCT_INACTIVE_US 30000
...
int ivpu_jsm_trace_get_config(struct ivpu_device *vdev, u32 *trace_level,
			      u32 *trace_destination_mask, u64 *trace_hw_component_mask)
{
	struct vpu_jsm_msg req = { .type = VPU_JSM_MSG_TRACE_GET_CONFIG };
	struct vpu_jsm_msg resp;
	int ret;

	ret = ivpu_ipc_send_receive(vdev, &req, VPU_JSM_MSG_TRACE_GET_CONFIG_RSP, &resp,
				    VPU_IPC_CHAN_ASYNC_CMD, vdev->timeout.jsm);
	if (ret) {
		ivpu_warn_ratelimited(vdev, "Failed to get config: %d", ret);
		return ret;
	}

	*trace_level = resp.payload.trace_config.trace_level;
	*trace_destination_mask = resp.payload.trace_config.trace_destination_mask;
	*trace_hw_component_mask = resp.payload.trace_config.trace_hw_component_mask;
				   ...
				   u32 *sample_size)
{
	...
	ret = ivpu_ipc_send_receive(vdev, &req, VPU_JSM_MSG_METRIC_STREAMER_START_DONE, &resp,
				    VPU_IPC_CHAN_ASYNC_CMD, vdev->timeout.jsm);
	if (ret) {
		ivpu_warn_ratelimited(vdev, "Failed to start metric streamer: ret %d", ret);
		return ret;
	}
...
int ivpu_jsm_metric_streamer_stop(struct ivpu_device *vdev, u64 metric_group_mask)
{
	...
	ret = ivpu_ipc_send_receive(vdev, &req, VPU_JSM_MSG_METRIC_STREAMER_STOP_DONE, &resp,
				    VPU_IPC_CHAN_ASYNC_CMD, vdev->timeout.jsm);
	if (ret)
		ivpu_warn_ratelimited(vdev, "Failed to stop metric streamer: ret %d", ret);
				    ...
				    u64 buffer_addr, u64 buffer_size, u64 *bytes_written)
{
	...
	ret = ivpu_ipc_send_receive(vdev, &req, VPU_JSM_MSG_METRIC_STREAMER_UPDATE_DONE, &resp,
				    VPU_IPC_CHAN_ASYNC_CMD, vdev->timeout.jsm);
	if (ret) {
		ivpu_warn_ratelimited(vdev, "Failed to update metric streamer: ret %d", ret);
		return ret;
	}
...
int ivpu_jsm_metric_streamer_info(struct ivpu_device *vdev, u64 metric_group_mask, u64 buffer_addr,
				  u64 buffer_size, u32 *sample_size, u64 *bytes_written)
{
	...
	ret = ivpu_ipc_send_receive(vdev, &req, VPU_JSM_MSG_METRIC_STREAMER_INFO_DONE, &resp,
				    VPU_IPC_CHAN_ASYNC_CMD, vdev->timeout.jsm);
	if (ret) {
		ivpu_warn_ratelimited(vdev, "Failed to get metric streamer info: ret %d", ret);
		return ret;
	}
			     ...
			     u64 cmdq_base, u32 cmdq_size)
{
	...
	ivpu_dbg(vdev, JSM, "Doorbell %u registered to context %u\n", db_id, ctx_id);
...
int
ivpu_jsm_hws_create_cmdq(struct ivpu_device *vdev, u32 ctx_id, u32 cmdq_group, u32 cmdq_id, u32 pid,
			 u32 engine, u64 cmdq_base, u32 cmdq_size)
{
					      ...
					      u32 priority)
{
	...
	req.payload.hws_set_context_sched_properties.context_quantum = 0;
	req.payload.hws_set_context_sched_properties.grace_period_same_priority = 0;
	...
	ret = ivpu_ipc_send_receive(vdev, &req, VPU_JSM_MSG_SET_CONTEXT_SCHED_PROPERTIES_RSP, &resp,
				    VPU_IPC_CHAN_ASYNC_CMD, vdev->timeout.jsm);
	if (ret)
		ivpu_warn_ratelimited(vdev, "Failed to set context scheduling properties: %d\n",
				      ret);
...
int ivpu_jsm_hws_set_scheduling_log(struct ivpu_device *vdev, u32 engine_idx, u32 host_ssid,
				    u64 vpu_log_buffer_va, u64 notify_index)
{
	...
	req.payload.hws_set_scheduling_log.notify_index = notify_index;
...
int ivpu_jsm_state_dump(struct ivpu_device *vdev)
{
	...
	return ivpu_ipc_send_receive_no_hb(vdev, &req, VPU_JSM_MSG_STATE_DUMP_RSP, &resp,
					   VPU_IPC_CHAN_ASYNC_CMD, 30);
}
...
int ivpu_jsm_dct_enable(struct ivpu_device *vdev)
{
	struct vpu_jsm_msg req = { .type = VPU_JSM_MSG_DCT_ENABLE };
	struct vpu_jsm_msg resp;

	req.payload.pwr_dct_control.dct_active_us = DCT_ACTIVE_US;
	req.payload.pwr_dct_control.dct_inactive_us = DCT_INACTIVE_US;

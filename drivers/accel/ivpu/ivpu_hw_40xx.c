...
#define BUTTRESS_ALL_IRQ_MASK ((BUTTRESS_IRQ_MASK) | \
			       (REG_FLD(VPU_40XX_BUTTRESS_INTERRUPT_STAT, FREQ_CHANGE)))
...
static void ivpu_hw_wa_init(struct ivpu_device *vdev)
{
	...
	if (ivpu_is_simics(vdev))
		vdev->wa.reset_during_power_up = true;

	IVPU_PRINT_WA(punit_disabled);
	IVPU_PRINT_WA(clear_runtime_mem);
	IVPU_PRINT_WA(disable_clock_relinquish);
	IVPU_PRINT_WA(reset_during_power_up);
}
...
static void ivpu_boot_pwr_island_delay_set(struct ivpu_device *vdev)
{
	u32 val, post, status;
...
static int ivpu_boot_pwr_domain_enable(struct ivpu_device *vdev)
{
	...
	if (ivpu_hw_gen(vdev) == IVPU_HW_50XX)
		ivpu_boot_pwr_island_delay_set(vdev);
...
static int ivpu_hw_40xx_info_init(struct ivpu_device *vdev)
{
	...
	hw->btrs_irq_mask = BUTTRESS_IRQ_MASK;

	if (ivpu_test_mode & IVPU_TEST_MODE_FREQ_CHANGE_ENABLE)
		hw->btrs_irq_mask = REG_SET_FLD(VPU_40XX_BUTTRESS_INTERRUPT_STAT, FREQ_CHANGE,
						hw->btrs_irq_mask);
...
static int ivpu_hw_40xx_power_up(struct ivpu_device *vdev)
{
	...
	if (IVPU_WA(reset_during_power_up)) {
...
static void ivpu_hw_40xx_dct_drive(struct ivpu_device *vdev, u32 dct_val)
{
	u32 val = REGB_RD32(VPU_40XX_BUTTRESS_PCODE_MAILBOX);

	val = REG_SET_FLD_NUM(VPU_40XX_BUTTRESS_PCODE_MAILBOX, CMD, DCT_REQ, val);
	val = REG_SET_FLD_NUM(VPU_40XX_BUTTRESS_PCODE_MAILBOX, PARAM1,
			      dct_val ? DCT_ENABLE : DCT_DISABLE, val);
	val = REG_SET_FLD_NUM(VPU_40XX_BUTTRESS_PCODE_MAILBOX, PARAM2, dct_val, val);

	REGB_WR32(VPU_40XX_BUTTRESS_PCODE_MAILBOX, val);
}
...
static void ivpu_hw_40xx_irq_clear(struct ivpu_device *vdev)
{
	...
	REGB_WR32(VPU_40XX_BUTTRESS_INTERRUPT_STAT, BUTTRESS_ALL_IRQ_MASK);
}
...
static void ivpu_hw_40xx_irq_enable(struct ivpu_device *vdev)
{
	...
	REGB_WR32(VPU_40XX_BUTTRESS_LOCAL_INT_MASK, (u32)(~vdev->hw->btrs_irq_mask));
...
static void ivpu_hw_40xx_surv_handler(struct ivpu_device *vdev)
{
	u32 val = REGB_RD32(VPU_40XX_BUTTRESS_PCODE_MAILBOX_SHADOW);
	u32 dct_val;

	if (!REG_TEST_FLD_NUM(VPU_40XX_BUTTRESS_PCODE_MAILBOX_SHADOW, CMD, DCT_REQ, val)) {
		ivpu_err_ratelimited(vdev, "Pcode request: 0x%x not allowed command", val);
		return;
	}

	if (REG_TEST_FLD_NUM(VPU_40XX_BUTTRESS_PCODE_MAILBOX_SHADOW, PARAM1,
			     DCT_ENABLE, val)) {
		dct_val = ivpu_dct_val;
	} else if (REG_TEST_FLD_NUM(VPU_40XX_BUTTRESS_PCODE_MAILBOX_SHADOW, PARAM1,
				    DCT_DISABLE, val)) {
		dct_val = 0;
	} else {
		ivpu_err_ratelimited(vdev, "DCT param1 wrong value\n");
		return;
	}

	ivpu_pm_dct_queue_work(vdev, dct_val);
}

/* Handler for IRQs from Buttress core (irqB) */
static bool ivpu_hw_40xx_irqb_handler(struct ivpu_device *vdev, int irq)
{
	u32 status = REGB_RD32(VPU_40XX_BUTTRESS_INTERRUPT_STAT) & vdev->hw->btrs_irq_mask;
	...
	if (REG_TEST_FLD(VPU_40XX_BUTTRESS_INTERRUPT_STAT, SURV_ERR, status)) {
		...
		ivpu_hw_40xx_surv_handler(vdev);
	}

	if (REG_TEST_FLD(VPU_40XX_BUTTRESS_INTERRUPT_STAT, FREQ_CHANGE, status))
		ivpu_dbg(vdev, IRQ, "FREQ_CHANGE irq: %08x", REGB_RD32(VPU_40XX_BUTTRESS_PLL_FREQ));
...
{
	bool freq_change_disabled = !(ivpu_test_mode & IVPU_TEST_MODE_FREQ_CHANGE_ENABLE);
	bool irqv_handled, irqb_handled, wake_thread = false;
	struct ivpu_device *vdev = ptr;

	REGB_WR32(VPU_40XX_BUTTRESS_GLOBAL_INT_MASK, 0x1);

	if (freq_change_disabled || !ivpu_hw_40xx_is_idle((vdev)))
...
static void ivpu_hw_40xx_diagnose_failure(struct ivpu_device *vdev)
{
	...
	u32 irqb = REGB_RD32(VPU_40XX_BUTTRESS_INTERRUPT_STAT) & vdev->hw->btrs_irq_mask;
...
const struct ivpu_hw_ops ivpu_hw_40xx_ops = {
	...
	.dct_drive = ivpu_hw_40xx_dct_drive,

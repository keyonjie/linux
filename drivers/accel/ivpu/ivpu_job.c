...
static struct ivpu_cmdq *ivpu_cmdq_alloc(struct ivpu_file_priv *file_priv, u16 engine, u8 priority)
{
...
static void ivpu_hws_cmdq_fini(struct ivpu_file_priv *file_priv, struct ivpu_cmdq *cmdq)
{
	ivpu_jsm_hws_destroy_cmdq(file_priv->vdev, file_priv->ctx.id, cmdq->db_id);
}
...
static void ivpu_unregister_db(struct ivpu_file_priv *file_priv, struct ivpu_cmdq *cmdq)
{
					   ...
					   u8 priority)
{
	...
	if (!cmdq) {
		cmdq = ivpu_cmdq_alloc(file_priv, engine, priority);
		if (!cmdq)
			return NULL;
...
static void ivpu_cmdq_release_locked(struct ivpu_file_priv *file_priv, u16 engine, u8 priority)
{
	...
	if (cmdq) {
		...
		if (cmdq->db_registered) {
			if (file_priv->vdev->hw->sched_mode == VPU_SCHEDULING_MODE_HW)
				ivpu_hws_cmdq_fini(file_priv, cmdq);
			ivpu_unregister_db(file_priv, cmdq);
		}

...
static void ivpu_hw_wa_init(struct ivpu_device *vdev)
{
	...
	/* UPSTREAM: Remove comment: WA for HW hang, see HSD-ES 18027724960 */
...
static int ivpu_hw_37xx_info_init(struct ivpu_device *vdev)
{
	...
	if (ivpu_test_mode & IVPU_TEST_MODE_FREQ_CHANGE_ENABLE)
		hw->btrs_irq_mask = REG_SET_FLD(VPU_37XX_BUTTRESS_INTERRUPT_STAT, FREQ_CHANGE,
						hw->btrs_irq_mask);
...
static void ivpu_hw_37xx_irqb_clear(struct ivpu_device *vdev, u32 status)
{
...
static void ivpu_hw_37xx_irq_clear(struct ivpu_device *vdev)
{
	...
	ivpu_hw_37xx_irqb_clear(vdev, BUTTRESS_ALL_IRQ_MASK);
}
...
static void ivpu_hw_37xx_irq_enable(struct ivpu_device *vdev)
{
	...
	REGB_WR32(VPU_37XX_BUTTRESS_LOCAL_INT_MASK, (u32)(~vdev->hw->btrs_irq_mask));
...
{
	u32 status = REGB_RD32(VPU_37XX_BUTTRESS_INTERRUPT_STAT) & vdev->hw->btrs_irq_mask;
	...
	ivpu_hw_37xx_irqb_clear(vdev, status);
...
static void ivpu_hw_37xx_diagnose_failure(struct ivpu_device *vdev)
{
	...
	u32 irqb = REGB_RD32(VPU_37XX_BUTTRESS_INTERRUPT_STAT) & vdev->hw->btrs_irq_mask;
